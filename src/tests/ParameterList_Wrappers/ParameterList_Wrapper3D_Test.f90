...
if(allocated(I1Parray))  deallocate(I1Parray);  allocate(I1Parray(1,1,2));  I1Parray  = 1
if(allocated(I2Parray))  deallocate(I2Parray);  allocate(I2Parray(2,1,1));  I2Parray  = 2
if(allocated(I4Parray))  deallocate(I4Parray);  allocate(I4Parray(1,2,1));  I4Parray  = 4
if(allocated(I8Parray))  deallocate(I8Parray);  allocate(I8Parray(3,1,1));  I8Parray  = 8
if(allocated(R4Parray))  deallocate(R4Parray);  allocate(R4Parray(1,3,1));  R4Parray  = 0.4
if(allocated(R8Parray))  deallocate(R8Parray);  allocate(R8Parray(4,1,1));  R8Parray  = 0.8
if(allocated(Larray))    deallocate(Larray);    allocate(Larray(1,4,1));    Larray    = .true.
if(allocated(DLCAarray)) deallocate(DLCAarray); allocate(character(len=6):: DLCAarray(1,5,1)); DLCAarray = 'String'
...
write(unit=OUTPUT_UNIT, fmt='(A,3I4)') 'I1P Shape:',  Parameters%GetShape(Key='I1P')
write(unit=OUTPUT_UNIT, fmt='(A,3I4)') 'I2P Shape:',  Parameters%GetShape(Key='I2P')
write(unit=OUTPUT_UNIT, fmt='(A,3I4)') 'I4P Shape:',  Parameters%GetShape(Key='I4P')
write(unit=OUTPUT_UNIT, fmt='(A,3I4)') 'I8P Shape:',  Parameters%GetShape(Key='I8P')
write(unit=OUTPUT_UNIT, fmt='(A,3I4)') 'R4P Shape:',  Parameters%GetShape(Key='R4P')
write(unit=OUTPUT_UNIT, fmt='(A,3I4)') 'R8P Shape:',  Parameters%GetShape(Key='R8P')
write(unit=OUTPUT_UNIT, fmt='(A,3I4)') 'L Shape:',    Parameters%GetShape(Key='L')
write(unit=OUTPUT_UNIT, fmt='(A,3I4)') 'DLCA Shape:', Parameters%GetShape(Key='DLCA')
